...
** ID     : 161720110
** Class  : 1617204
** Github : https://github.com/Telecommunication/Exper.3/lte.cc 
*****************************************************************************/
...
bool AreOverlapping (Box a, Box b) //判断两个区域是否有覆盖
{
...
static ns3::GlobalValue g_useUdp ("useUdp",
                                  ...
                                  "If EPC is not used, this parameter will be ignored.",
                                  ns3::BooleanValue (true), // 应用层使用UDP协议进行数据传输
                                  ns3::MakeBooleanChecker ());
...
static ns3::GlobalValue g_outdoorUeMinSpeed ("outdoorUeMinSpeed",
                                             ...
                                             ns3::DoubleValue (1.0), //宏用户最小速度值（随机移动模型）
                                             ns3::MakeDoubleChecker<double> ());
...
main (int argc, char *argv[])
{
  ...
  if (nMacroEnbSites > 0)
    {
      ...
      cout << "nMacroEnbSitesY = " << nMacroEnbSitesY << endl;
  ...
  cout << "nHomeEnbs = " << nHomeEnbs << endl;
  uint32_t nHomeUes = round (nHomeEnbs * homeUesHomeEnbRatio);
  cout << "nHomeUes = " << nHomeUes << endl;
  ...
  cout << "nMacroUes = " << nMacroUes << " (density=" << macroUeDensity << ")" << endl;
...
//   LogComponentEnable ("PfFfMacScheduler", LOG_LEVEL_ALL);
  ...
  lteHelper->SetEnbAntennaModelType ("ns3::IsotropicAntennaModel"); // 各向同性天线模型
  ...
  lteHelper->SetEnbDeviceAttribute ("CsgId", UintegerValue (1)); // csg编号
  ...
  // 建立宏用户
  // macro Ues
  cout << "randomly allocating macro UEs in " << macroUeBox << " speedMin " << outdoorUeMinSpeed << " speedMax " << outdoorUeMaxSpeed << endl;
  if (outdoorUeMaxSpeed!=0.0) // 设定移动参数
    {
  ...
  if (epc)
    {
      cout << "setting up internet and remote host" << endl;
      ...
      Ipv4StaticRoutingHelper ipv4RoutingHelper;
      Ptr<Ipv4StaticRouting> remoteHostStaticRouting = ipv4RoutingHelper.GetStaticRouting (remoteHost->GetObject<Ipv4> ()); // 建立静态路由
      remoteHostStaticRouting->AddNetworkRouteTo (Ipv4Address ("7.0.0.0"), Ipv4Mask ("255.0.0.0"), 1);

      // for internetworking purposes, consider together home UEs and macro UEs
      ues.Add (homeUes); // 将宏用户与家庭用户一同考虑
      ues.Add (macroUes);
  ...
  if (epc)
    {
      cout << "setting up applications" << endl;
      ...
      for (uint32_t u = 0; u < ues.GetN (); ++u)
        {
          ...
          for (uint32_t b = 0; b < numBearersPerUe; ++b) // 承载
            {
              ++dlPort;
              ++ulPort;
              ...
              if (useUdp) // 设置udp数据发送
                {
                  ...
                  if (epcUl)
                    {
                      cout << "installing UDP UL app for UE " << u << endl;
